...
- Merged **`AGENTS.md`** into **`CLAUDE.md`** (maintainer, Konflux, and agent context in one file); **`.github/PULL_REQUEST_TEMPLATE.md`** and contributor/issue links updated; LLM workflow suggests commits with **`Assisted-By:`** / **`Signed-off-by:`** trailers; **`.github/commit-message-template.txt`** added as a blank layout
...
- **`AGENTS.md` (since merged into `CLAUDE.md`)** added a **Canonical upload entry** subsection: primary path is **`PulpHelper`** (orchestrator + `upload_collect`); **`UploadService`** is a thin wrapper for tests and programmatic callers
