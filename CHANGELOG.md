...
### Fixed
- **RPM `signed_by` label values containing commas:** Pulp `pulp_label_select` splits on comma before parsing terms, so server-side filters broke for those values; `get_rpm_by_checksums_and_signed_by` now uses chunked `pkgId__in` plus client-side label filtering; `get_rpm_by_signed_by` paginates without broken `q` when values contain commas; filename+signed_by combined path uses NVR query then client-side filter; tests in `tests/api/pulp_client/test_tasks_rpm.py`
...
- **`upload --signed-by`:** help text notes that values may include commas and should be quoted in the shell when needed
- **`docs/cli-reference.md`:** `upload` / `search-by` sections describe comma-containing `signed_by` values, shell quoting, and client-side exact label matching when server-side filters are ambiguous; **`AGENTS.md`:** convention to update this doc when changing flags under `pulp_tool/cli/`; **`search-by --signed-by`:** help text aligned with upload (commas, quoting)
