...
- Builds one **large** RPM (`test.large`) under `large/x86_64/` for multipart upload timeout coverage (default 25 MiB payload)

**Usage:**
```bash
./pre-test.py --build-dir <directory> [--large-rpm-size-mb 25]
```

**Arguments:**
- `--build-dir`: Directory where test packages will be built (default: current directory)
- `--large-rpm-size-mb`: Payload size for the large upload test RPM (default: 25; use `0` to skip)
...
- ✅ Large RPM upload against a real Pulp server (multipart payload; checksum verified via `search-by`)
