...
class TestParseJsonResponse:
    ...
    def test_parse_json_skip_success_check(self) -> None:
    ...
    def test_parse_json_non_success_status(self) -> None:
...
class TestExtractTaskHref:
    ...
    def test_extract_task_href_success(self) -> None:
    ...
    def test_extract_task_href_missing_key(self) -> None:
    ...
    def test_extract_task_href_invalid_json(self) -> None:
...
class TestExtractCreatedResources:
    ...
    def test_extract_created_resources_success(self) -> None:
    ...
    def test_extract_created_resources_empty(self) -> None:
        """Test extracting when no resources were created."""
        task_response = TaskResponse(pulp_href="/pulp/api/v3/tasks/123/", state="completed", created_resources=[])
        result = extract_created_resources(task_response, "test operation")
        assert result == []

    def test_extract_created_resources_none(self) -> None:
        """Test extracting when created_resources is None."""
        task_response = TaskResponse(pulp_href="/pulp/api/v3/tasks/123/", state="completed")
        result = extract_created_resources(task_response, "test operation")
        assert result == []
...
class TestCheckTaskSuccess:
    ...
    def test_check_task_success_completed(self) -> None:
        """Test successful task check."""
        task_response = TaskResponse(pulp_href="/pulp/api/v3/tasks/123/", state="completed")
        result = check_task_success(task_response, "test operation")
        assert result is True

    def test_check_task_success_failed_with_error(self) -> None:
        """Test failed task with error description."""
        task_response = TaskResponse(
            pulp_href="/pulp/api/v3/tasks/123/", state="failed", error={"description": "Database connection failed"}
        )
    ...
    def test_check_task_success_failed_no_error(self) -> None:
        """Test failed task without error description."""
        task_response = TaskResponse(pulp_href="/pulp/api/v3/tasks/123/", state="failed")
...
class TestExtractResultsList:
    ...
    def test_extract_results_list_success(self) -> None:
        """Test extracting results list from response."""
        response = httpx.Response(
            200, json={"results": [{"id": 1, "name": "test1"}, {"id": 2, "name": "test2"}], "count": 2}
        )
        result = extract_results_list(response, "search operation")
        assert len(result) == 2
        assert result[0]["id"] == 1

    def test_extract_results_list_empty_allowed(self) -> None:
    ...
    def test_extract_results_list_empty_not_allowed(self) -> None:
    ...
    def test_extract_results_list_missing_results_key(self) -> None:
...
class TestContentFindResultsFromResponse:
    ...
    def test_success_paginated(self) -> None:
    ...
    def test_empty_body(self) -> None:
        ...
        response = httpx.Response(200, content=b"", request=httpx.Request("GET", "https://pulp.example.com/content/"))
    ...
    def test_non_success(self) -> None:
        response = httpx.Response(
            502, content=b"bad gateway", request=httpx.Request("GET", "https://pulp.example.com/content/")
        )
        with pytest.raises(ValueError, match="Response not successful"):
            content_find_results_from_response(response, "test")
    ...
    def test_invalid_json(self) -> None:
        response = httpx.Response(
            200, content=b"not json", request=httpx.Request("GET", "https://pulp.example.com/content/")
        )
...
class TestExtractSingleResult:
    ...
    def test_extract_single_result_success(self) -> None:
        """Test extracting single result from response."""
        response = httpx.Response(200, json={"results": [{"id": 1, "name": "test"}], "count": 1})
        result = extract_single_result(response, "get operation")
        assert result == {"id": 1, "name": "test"}

    def test_extract_single_result_empty(self) -> None:
...
class TestGetResponseField:
    ...
    def test_get_response_field_exists(self) -> None:
    ...
    def test_get_response_field_missing_with_default(self) -> None:
