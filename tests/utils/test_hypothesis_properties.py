...
def test_resolve_correlation_id_prefers_config_then_env(cfg, env, ns, bid):
    """Config ``correlation_id`` wins; then env; then derived ``ns/bid`` or ``bid``."""
    # Avoid pytest ``monkeypatch`` here: Hypothesis reuses the test function across examples.
...
def test_strip_namespace_from_build_id_roundtrip(parts):
...
def test_sanitize_build_id_for_repository_no_invalid_chars(seg):
...
@given(
    ...
    ver=st.from_regex(r"[0-9]+(\.[0-9]+)?", fullmatch=True),
    rel=st.from_regex(r"[0-9]+[a-z0-9.]*", fullmatch=True),
    arch=st.sampled_from(["x86_64", "noarch", "src", "aarch64"]),
)
def test_parse_rpm_filename_nvra_roundtrip(name, ver, rel, arch):
...
def test_versions_from_status_payload_extracts_pairs(versions):
...
def test_versions_from_status_payload_ignores_non_dict_entries():
