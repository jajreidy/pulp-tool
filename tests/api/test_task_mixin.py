...
class TestTaskMixin:
    ...
    def test_wait_for_finished_task_timeout(self, mock_pulp_client, httpx_mock) -> None:
        ...
        def mock_get_task(href) -> None:
            return TaskResponse(pulp_href=href, state="running", result=None)
    ...
    def test_list_tasks(self, mock_pulp_client, httpx_mock) -> None:
        """Test list_tasks method (lines 76,78-79)."""
        httpx_mock.get("https://pulp.example.com/pulp/api/v3/test-domain/api/v3/tasks/").mock(
            return_value=httpx.Response(
                200,
                json={
                    "results": [{"pulp_href": "/api/v3/tasks/12345/", "state": "completed", "result": {}}],
                    "next": None,
                    "previous": None,
                    "count": 1,
                },
