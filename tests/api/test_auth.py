...
class TestOAuth2ClientCredentialsAuth:
    ...
    def test_auth_flow_without_token(self, httpx_mock) -> None:
    ...
    def test_auth_flow_with_expired_token(self, httpx_mock) -> None:
    ...
    def test_auth_flow_with_401_retry(self, httpx_mock) -> None:
    ...
    def test_retrieve_token_success(self, httpx_mock) -> None:
    ...
    def test_retrieve_token_invalid_response(self, httpx_mock) -> None:
        """Test token retrieval with invalid response format."""
        auth = OAuth2ClientCredentialsAuth(
            client_id="test-client", client_secret="test-secret", token_url="https://test.com/token"
        )
        httpx_mock.post("https://test.com/token").mock(return_value=httpx.Response(200, json={"invalid": "response"}))
        with pytest.raises(ValueError, match="Invalid token response format"):
            auth._retrieve_token()

    def test_retrieve_token_http_error(self, httpx_mock) -> None:
...
class TestOAuth2Integration:
    ...
    def test_auth_with_real_request(self, httpx_mock) -> None:
        """Test authentication flow with a real-looking request."""
        auth = OAuth2ClientCredentialsAuth(
            client_id="test-client", client_secret="test-secret", token_url="https://test.com/token"
        )
        httpx_mock.post("https://test.com/token").mock(
            return_value=httpx.Response(200, json={"access_token": "valid-token", "expires_in": 7200})
        )
        httpx_mock.get("https://api.example.com/data").mock(return_value=httpx.Response(200, json={"data": "test"}))
